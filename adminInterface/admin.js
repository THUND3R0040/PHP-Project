...
users.addEventListener("click", () => {
  ...
  container[0].innerHTML = "<article class='stats'>  <div class='swiper mySwiper'><div class='swiper-wrapper'></div></div></article>";
  UpdatePage("users");
});
  ...
  xhr.onload = function () {
    if (this.status === 200) {
      ...
      if(str == "dashboard"){
        ...
        container[0].innerHTML = response.htmlContent;
        showCharts(response.sales,response.nbOrders);
      }
      else if(str == "users"){
        console.log(response.htmlContent);
        document.querySelector(".swiper-wrapper").innerHTML = response.htmlContent;
        showSwiper();
        selectDelete();
        showForm();
        console.log(email);
...
const deleteComments = ()=>{
  ...
  deleteBtns.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      let id = e.target.parentElement.parentElement.childNodes[1].childNodes[1].textContent;
      deleteComment(id);
    })
  })
}
...
const productEditShow = ()=>{
  ...
  editBtns.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      let productName = e.target.nextElementSibling.childNodes[3].childNodes[1].textContent; 
...
const updateProductInfo = (name,newName,newPrice,newType) => {
  ...
  xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
  let msg = "name=" + name + "&newName=" + newName + "&newPrice=" + newPrice + "&newType=" + newType;
  xhr.send(msg);
}
...
const productDeleteShow = ()=>{
  ...
  deleteBtns.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      let id = e.target.parentElement.previousElementSibling.childNodes[3].childNodes[1].textContent;
      deleteProduct(id);
    })
  })
}
...
const deleteSale = (id) => {
  const xhr = new XMLHttpRequest();
  xhr.open('POST', 'deleteSale.php', true);
  xhr.onload = function () {
    if (this.status === 200) {
      if(this.responseText == "success"){
        UpdatePage("sales");
...
const deleteSales = ()=>{
  ...
  salesDelete.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      let id = e.target.parentElement.parentElement.childNodes[1].childNodes[1].textContent.substr(1);
      deleteSale(id);
    })
  })
}
...
const updateUser = (email,newEmail,newRole,newName) => {
  ...
  xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
  let msg = "email=" + email + "&newEmail=" + newEmail + "&newRole=" + newRole + "&newName=" + newName;
  xhr.send(msg);
}
